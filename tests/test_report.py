        ...
        if isinstance(node, ast.Assign):
            for t in node.targets:
                ...
                if isinstance(t, ast.Subscript) and isinstance(t.slice, ast.Constant) and \
                    ...
                    if s is not None:
                        direct.append(s)
        if isinstance(node, ast.FunctionDef) and node.name in ("house_block", "refusal"):
...
def test_every_refusal_string_pm_can_emit_today_has_a_rule(rp):
    ...
    assert set(indirect) <= {"book['day']['halt_reason']", "ladder['reason']", "pnote", "b",
                             "'; '.join(p['warnings'])", "hb",
                             # P-06: earnings.refusal() builds the string; earnings.py is
                             # harvested below so the text is still checked against a rule.
                             "earnings_mod.refusal(r['ticker'], einfo, _earnings_window())"}, indirect
    for name in ("portfolio.py", "broker_policy.py", "ladder.py", "earnings.py"):
