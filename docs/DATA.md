...
| **10-K / 10-Q text change (E16)** | **EDGAR `data.sec.gov/submissions` → Archives primary document; `engine/filings.py`** | **scan rows (`features.filing_*`, not scored)** | **after each filing; a fetch step on the box, then the batch CLI** | **10 req/s, `User-Agent` mandatory — see §4** | **Filing-dated; `features_for` nulls anything filed after the scan date — see §4** |
...
## 4. 10-K / 10-Q text change — "Lazy Prices" (P-02, E16)
...
floor and the promotion rule are in `docs/BACKTEST.md` §6d. What this cannot tell you: whether
